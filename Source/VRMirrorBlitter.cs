        ...
        public override void Draw()
        {
            ...
            switch (MirrorMode)
            {
                case MirrorTextureEyeSource.BothEyes:
                    float w = size.X * 0.5f;
                    BlitEye(_context.LeftEyeRenderTarget, 0, 0, w, size.Y, true, false);
                    BlitEye(_context.RightEyeRenderTarget, w, 0, w, size.Y, false, true);
                    break;
                case MirrorTextureEyeSource.LeftEye:
                    BlitEye(_context.LeftEyeRenderTarget, 0, 0, size.X, size.Y);
                    break;
                case MirrorTextureEyeSource.RightEye:
                    BlitEye(_context.RightEyeRenderTarget, 0, 0, size.X, size.Y);
                    break;
            }
        }

        private void BlitEye(RenderTarget rt, float x, float y, float w, float h, bool horizontalRight = false, bool horizontalLeft = false)
        {
            float rtAspectRatio = rt.Width / (float)rt.Height;
            ...
            Render2D.DrawRenderTarget(rt, new Rectangle(posX, posY, sampleWidth, sampleHeight), Color.White);
        }
    }
}
